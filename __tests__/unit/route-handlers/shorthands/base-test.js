...
describe('Unit | Route handlers | Shorthands | BaseShorthandRouteHandler', function() {
  ...
  test('getModelClassFromPath works with various named route path variable', () => {
    ...
    expect(handler.getModelClassFromPath(urlWithIdAndSlash, true)).toEqual('fancy-user');

    urlWithSlash = '/api/exquisite-users';
    urlWithIdAndSlash = '/api/exquisite-users/:objectId';

    expect(handler.getModelClassFromPath(urlWithSlash)).toEqual('exquisite-user');
    expect(handler.getModelClassFromPath(urlWithIdAndSlash, true)).toEqual('exquisite-user');
    ...
    expect(handler.getModelClassFromPath(urlWithIdAndSlash, true)).toEqual('elegant-user');
  });
