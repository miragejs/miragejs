...
module('Integration | ORM | update', function(hooks) {
  ...
  test('a collection can update its models with a key and value', assert => {
    ...
    expect(collection.models[0].attrs).toEqual({ id: '1', name: 'Link', location: 'Hyrule', evil: true });
  });
  ...
  test('it can update its models with a hash of attrs', assert => {
    ...
    expect(collection.models[0].attrs).toEqual({ id: '1', name: 'Link', location: 'The water temple', evil: true });
    expect(collection.models[1].attrs).toEqual({ id: '2', name: 'Zelda', location: 'The water temple', evil: true });
  });
  ...
  test('it can set an attribute and then save the model', assert => {
    ...
    expect(user.attrs).toEqual({ id: '1', name: 'Young link', location: 'Hyrule', evil: false });
    expect(this.db.users.find(1)).toEqual({ id: '1', name: 'Link', location: 'Hyrule', evil: false });

    user.save();

    expect(user.attrs).toEqual({ id: '1', name: 'Young link', location: 'Hyrule', evil: false });
    expect(this.db.users.find(1)).toEqual({ id: '1', name: 'Young link', location: 'Hyrule', evil: false });
  });
  ...
  test('it can update and immediately persist a single attribute', assert => {
    ...
    expect(link.attrs).toEqual({ id: '1', name: 'Link', location: 'Hyrule', evil: true });
    expect(this.db.users.find(1)).toEqual({ id: '1', name: 'Link', location: 'Hyrule', evil: true });
  });
  ...
  test('it can update a hash of attrs immediately', assert => {
    ...
    expect(link.attrs).toEqual({ id: '1', name: 'Evil link', location: 'Hyrule', evil: true });
    expect(this.db.users.find(1)).toEqual({ id: '1', name: 'Evil link', location: 'Hyrule', evil: true });
  });
  ...
  test('it can update a non-existing attribute', assert => {
    ...
    expect(link.attrs).toEqual(
      { id: '1', name: 'Evil link', location: 'Hyrule', evil: true, reallyEvil: 'absolutely evil' }
    );
  ...
  test('if users sets incorrectly an attribute without using update, it will still work', assert => {
    ...
    expect(link.attrs).toEqual(
      { id: '1', reallyEvil: 'a little flower', evil: true, location: 'Hyrule', name: 'Link' }
    );
    expect(this.db.users.find(1)).toEqual(
      { id: '1', reallyEvil: 'a little flower', evil: true, location: 'Hyrule', name: 'Link' }
    );
