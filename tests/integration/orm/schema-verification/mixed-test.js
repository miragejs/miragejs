...
module("Integration | ORM | Schema Verification | Mixed", function() {
  ...
  test("multiple has-many associations of the same type", function(assert) {
    ...
    assert.deepEqual(
      post.inverseFor(notesAssociation),
      post.associationFor("author")
    );
    ...
    assert.deepEqual(
      post.inverseFor(messagesAssociation),
  ...
  test("one-to-many reflexive association is correct", function(assert) {
    ...
    assert.deepEqual(
      frodo.inverseFor(parentAssociation),
  ...
  test("one-to-many polymorphic association is correct", function(assert) {
    ...
    assert.deepEqual(
      post.inverseFor(writingsAssociation),
      postAuthorAssociation
    );
    assert.deepEqual(
      article.inverseFor(writingsAssociation),
      articleAuthorAssociation
    );
    assert.deepEqual(
      author.inverseFor(postAuthorAssociation),
      writingsAssociation
    );
    assert.deepEqual(
      author.inverseFor(postAuthorAssociation),
  ...
  test("multiple explicit inverse associations with the same key but different models does not throw an error", function(assert) {
    ...
    assert.deepEqual(
      post.inverseFor(userAuthoredPostsAssociation),
      post.associationFor("authors")
    );
    assert.deepEqual(
      book.inverseFor(userAuthoredBooksAssociation),
      book.associationFor("authors")
    );
    assert.deepEqual(
      frodo.inverseFor(postsAuthorsAssociation),
      frodo.associationFor("authoredPosts")
    );
    assert.deepEqual(
      frodo.inverseFor(bookAuthorsAssociation),
