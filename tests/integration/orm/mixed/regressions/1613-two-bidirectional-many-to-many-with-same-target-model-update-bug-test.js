...
module(
  "Integration | ORM | Mixed | Regressions | 1613 Two bidirectional one-to-many relationships with same target model update ids bug",
  function(hooks) {
    ...
    test(`it works, and all inverses are correctly updated`, function(assert) {
      ...
      assert.deepEqual(this.db.posts.find(1), {
        id: "1",
        authorId: "1",
        editorId: "1"
      });
      assert.deepEqual(this.db.users.find(1), {
        id: "1",
        authoredPostIds: ["1"],
        editedPostIds: ["1"]
      });
