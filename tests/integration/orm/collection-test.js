...
module("Integration | ORM | collection", function(hooks) {
  ...
  test("a collection can filter its models", function(assert) {
    ...
    assert.equal(
      ...
      "user",
      "the filtered collection has the right type"
    );
  ...
  test("a collection can sort its models", function(assert) {
    let collection = this.schema.users.all();
    assert.deepEqual(collection.models.map(m => m.name), [
      "Link",
      "Zelda",
      "Ganon"
    ]);
    ...
    assert.equal(
      newCollection.modelName,
      "user",
      "the sorted collection has the right type"
    );
    assert.deepEqual(newCollection.models.map(m => m.name), [
      "Ganon",
      "Link",
      "Zelda"
    ]);
  ...
  test("a collection can slice its models", function(assert) {
    let collection = this.schema.users.all();
    assert.deepEqual(
      collection.models.map(m => m.name),
      ["Link", "Zelda", "Ganon"],
      "Starts with 3"
    );
    ...
    assert.equal(
      newCollection.modelName,
      "user",
      "the sliced collection has the right type"
    );
