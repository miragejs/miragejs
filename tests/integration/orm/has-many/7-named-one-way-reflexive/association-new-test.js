...
module(
  "Integration | ORM | Has Many | Named One-Way Reflexive | association #new",
  function(hooks) {
  ...
  */
    ...
    states.forEach(state => {
      test(`a ${state} can build a new associated child`, function(assert) {
        ...
        assert.equal(
          blueTag.labels.models.length,
          0,
          "the inverse was not set"
        );
        ...
        assert.deepEqual(
          blueTag.attrs,
          { id: blueTag.id, name: "Blue", labelIds: [] },
          "the child was persisted"
        );
        assert.equal(
          tag.labels.models.length,
          initialCount + 1,
          "the collection size was increased"
        );
        assert.ok(
          tag.labels.includes(blueTag),
          "the model was added to tag.labels"
        );
        assert.ok(
          tag.labelIds.indexOf(blueTag.id) > -1,
