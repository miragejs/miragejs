...
module(
  "Integration | ORM | Has Many | Reflexive | association #create",
  function(hooks) {
    ...
    states.forEach(state => {
      test(`a ${state} can create an associated child`, function(assert) {
        ...
        assert.equal(
          ...
          initialCount + 2,
          "the collection size was increased"
        );
        assert.ok(
          tag.tags.includes(orangeTag),
          "the model was added to tag.tags"
        );
        assert.ok(
          tag.tags.includes(blueTag),
          "the model was added to tag.tags"
        );
        assert.ok(
          tag.tagIds.indexOf(orangeTag.id) > -1,
          "the id was added to the fks array"
        );
        assert.ok(
          tag.tagIds.indexOf(blueTag.id) > -1,
          "the id was added to the fks array"
        );
        assert.ok(
          tag.attrs.tagIds.indexOf(orangeTag.id) > -1,
          "fks were persisted"
        );
        assert.ok(
          tag.attrs.tagIds.indexOf(blueTag.id) > -1,
          "fks were persisted"
        );
