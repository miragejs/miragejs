...
module("Integration | ORM | Has Many | Named | association #new", function(
  hooks
) {
  ...
  */
  ...
  states.forEach(state => {
    test(`a ${state} can build a new associated parent`, function(assert) {
      ...
      assert.deepEqual(
        post.attrs,
        { id: post.id, title: "Lorem ipsum" },
        "the child was persisted"
      );
      assert.equal(
        user.blogPosts.models.length,
        initialCount + 1,
        "the collection size was increased"
      );
      assert.deepEqual(
        user.blogPosts.models.filter(a => a.id === post.id)[0],
        post,
        "the model was added to user.blogPosts"
      );
      assert.ok(
        user.blogPostIds.indexOf(post.id) > -1,
