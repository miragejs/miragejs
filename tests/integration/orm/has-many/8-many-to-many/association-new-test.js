...
module(
  "Integration | ORM | Has Many | Many to Many | association #new",
  function(hooks) {
  ...
  */
    ...
    states.forEach(state => {
      test(`a ${state} can build a new associated child`, function(assert) {
        ...
        assert.equal(
          blueProduct.orders.models.length,
          1,
          "the inverse was set"
        );
        ...
        assert.deepEqual(
          blueProduct.attrs,
          { id: blueProduct.id, name: "Blue", orderIds: [order.id] },
        ...
        assert.ok(
          order.products.includes(blueProduct),
          "the model was added to order.products"
        );
        assert.ok(
          order.productIds.indexOf(blueProduct.id) > -1,
