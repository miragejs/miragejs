...
module(
  "Unit | Route handlers | Shorthands | BaseShorthandRouteHandler",
  function(hooks) {
    ...
    test("it returns a number if it's a number", function(assert) {
      ...
      assert.equal(
        this.handler._getIdForRequest(this.request),
        2,
        "it returns a number"
      );
    });
    ...
    test("getModelClassFromPath works with various named route path variable", function(assert) {
      ...
      assert.equal(
        this.handler.getModelClassFromPath(urlWithSlash),
        "fancy-user",
        "it returns a singular model name"
      );
      assert.equal(
        this.handler.getModelClassFromPath(urlWithIdAndSlash, true),
        "fancy-user",
        "it returns a singular model name"
      );
    ...
    test("it can read the id from the url", function(assert) {
      ...
      assert.equal(
        this.handler._getIdForRequest(request),
        "test-id",
        "it returns id from url parameters."
      );
    });
    ...
    test("it can read the id from the request body", function(assert) {
      ...
      assert.equal(
        this.handler._getIdForRequest(request, jsonApiDoc),
        "jsonapi-id",
        "it returns id from json api data."
      );
    });
  }
);
