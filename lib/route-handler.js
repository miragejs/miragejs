...
function createHandler({
  ...
  let _originalHandler = handler.handle;
  handler.handle = function(request) {
    let response = {};

    const preMiddlewareRequest = [...middlewares.pre].reduce(
      (finalRequest, mw) => mw(finalRequest, response),
      request
    );

    response = _originalHandler.call(handler, preMiddlewareRequest);

    const postMiddlewareResponse = [...middlewares.post].reduce(
      (finalResponse, mw) => mw(request, finalResponse),
      response
    );

    return postMiddlewareResponse;
  };
