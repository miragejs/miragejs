...
class Serializer {
  ...
  constructor(registry, type, request = {}, server) {
  ...
  */
  ...
  /**
    ...
    export default Serializer.extend({
      keyForCollection(modelName) {
        return this._server.inflector.pluralize(dasherize(modelName));
      }
    });
    ```
   ...
   */
  keyForCollection(modelName) {
    return this._server.inflector.pluralize(this.keyForModel(modelName));
  }
    ...
    if (this.serializeIds === 'always') {
      model.associationKeys.forEach((key) => {
        ...
        if (this.isCollection(resource)) {
          ...
          newHash[formattedKey] = model[`${this._server.inflector.singularize(key)}Ids`];
   ...
   */
  keyForRelationship(modelName) {
    return camelize(this._server.inflector.pluralize(modelName));
  }
  ...
  keyForRelationshipIds(relationshipName) {
    return `${this._server.inflector.singularize(camelize(relationshipName))}Ids`;
  }
