import { invokeMap, isEqual } from "lodash-es";
...
export default class PolymorphicCollection {
   ...
   */
  remove(model) {
    let [match] = this.models.filter(m => isEqual(m.attrs, model.attrs));
    if (match) {
   ...
   */
  includes(model) {
    return this.models.filter(m => isEqual(m.attrs, model.attrs)).length > 0;
  }
