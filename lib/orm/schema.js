...
export default class Schema {
  constructor(config = {}) {
    ...
    /**
      Creates a new Mirage database. See the `Db` docs for the db's API.
      @property db
      @type {Object}
      @public
    */
    ...
    /**
     * Registry of models and their associations
     * @type {Object}
     * @private
     */
    this._registry = {};
    ...
    /**
     * Registry of polymorphic associations
     * @type {Object}
     * @private
     */
     ...
     * @private
     */
    this._factoryManager = new FactoryManager(this);
    ...
    if (config.factories) {
      this._factoryManager.registerFactories(config.factories);
    }
   ...
   */
  registerModels(hash = {}) {
    forIn(hash, (modelClass, key) => {
      this.registerModel(key, modelClass);
    });
  }
   ...
   */
  factoryFor(type) {
    return this._factoryManager.factoryFor(type);
  }
  ...
  registerFactories(factoryMap) {
    this._factoryManager.registerFactories(factoryMap);
  }
   ...
   */
  build(type, ...options) {
    return this._factoryManager.build(type, ...options);
  }
   ...
   * @public
   */
  buildList(type, amount, ...options) {
    return this._factoryManager.buildList(type, amount, ...options);
  }

  /**
    Creates a new model instance with attributes *attrs*, including any overrides from *traits*, and inserts it into the database.

    ```js
    let post = schema.create('post', { title: 'Lorem ipsum' }, 'published');
  ...
  */
  create(type, ...options) {
    assert(
      ...
      `You called create('${type}') but no model or factory was found. Make sure you're passing in the singularized version of the model or factory name.`
    );
    ...
    if (this[this.toCollectionName(type)]) {
      let modelClass = this[this.toCollectionName(type)];

      modelOrRecord = modelClass.new(attrs).save();
    } else {
      ...
      if (collectionFromCreateList) {
        ...
        collectionName = this.toInternalCollectionName(type);
        collection = this.db[collectionName];
      }

      assert(
        collection,
        `You called create('${type}') but no model or factory was found.`
      );
  ...
  /**
    ...
    ```js
    let posts = schema.createList('post', 5, { title: 'Lorem ipsum' }, 'published');
    posts.length;       // 5
    posts[0].title;     // Lorem ipsum
    posts[0].id;        // 1
  ...
  */
  createList(type, amount, ...traitsAndOverrides) {
    assert(
      this._modelOrFactoryExistsForType(type),
      `You called createList('${type}') but no model or factory was found. Make sure you're passing in the singularized version of the model or factory name.`
    );
    assert(
      isInteger(amount),
    ...
    let list = [];
    let collectionName =
      this.toInternalCollectionName(type) ||
  ...
  */
  ...
  toCollectionName(type) {
    if (typeof collectionNameCache[type] !== "string") {
      ...
      const collectionName = camelize(this.inflector.pluralize(modelName));
  ...
  toModelName(type) {
    if (typeof modelNameCache[type] !== "string") {
      ...
      const modelName = this.inflector.singularize(dasherized);
  ...
  _modelOrFactoryExistsForType(type) {
    let modelExists = this.hasModelForModelName(type);
    let dbCollectionExists = this.db[this.toInternalCollectionName(type)];
  ...
  _modelOrFactoryExistsForTypeOrCollectionName(typeOrCollectionName) {
    let modelExists = this.hasModelForModelName(typeOrCollectionName);
    let dbCollectionExists =
      this.db[this.toInternalCollectionName(typeOrCollectionName)];
