...
export default class Schema {
  ...
  find(type, ids) {
    ...
    if (Array.isArray(ids)) {
      assert(
        ...
        `Couldn't find all ${this._container.inflector.pluralize(type)} with ids: (${ids.join(',')}) (found ${records.length} results, but was looking for ${ids.length})`
      );
    }
  ...
  */
  ...
  toInternalCollectionName(type) {
    return `_${this.toCollectionName(type)}`;
  }
