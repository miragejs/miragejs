...
export default class Schema {
  ...
  constructor(db, modelsMap = {}, server) {
    assert(db, 'A schema requires a db');
    assert(server, 'A schema requires a server reference');
    ...
    */
    ...
    // A reference for server instance
    this._server = server;
  }
  ...
  registerModel(type, ModelClass) {
    ...
    for (let associationProperty in ModelClass.prototype) {
      if (ModelClass.prototype[associationProperty] instanceof Association) {
        ...
        association._server = this._server;
  ...
  find(type, ids) {
    ...
    if (Array.isArray(ids)) {
      assert(
        ...
        `Couldn't find all ${this._server.inflector.pluralize(type)} with ids: (${ids.join(',')}) (found ${records.length} results, but was looking for ${ids.length})`
      );
    }
