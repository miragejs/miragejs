...
class Db {
  constructor(
    initialData,
    identityManagers = {},
    inflector = defaultInflector
  ) {
    this._collections = [];

    this.registerIdentityManagers(identityManagers);
    this.registerInflector(inflector);
  ...
  identityManagerFor(name) {
    return (
      this._identityManagers[this._inflector.singularize(name)] ||
  ...
  /**
   * @method registerInflector
   * @param {Object} inflector
   * @public
   * @hide
   */
  registerInflector(inflector) {
