...
export default class HasMany extends Association {
  ...
  get identifier() {
    return `${camelize(this._server.inflector.singularize(this.key))}Ids`;
  }
   ...
   */
  getForeignKey() {
    return `${this._server.inflector.singularize(camelize(this.key))}Ids`;
  }
  ...
  addMethodsToModelClass(ModelClass, key) {
    ...
    */
    modelPrototype[`new${capitalize(camelize(this._server.inflector.singularize(association.key)))}`] = function(...args) {
      let modelName, attrs;
      if (association.isPolymorphic) {
        modelName = args[0];
        attrs = args[1];
    ...
    */
    modelPrototype[`create${capitalize(camelize(this._server.inflector.singularize(association.key)))}`] = function(...args) {
      let modelName, attrs;
      if (association.isPolymorphic) {
        modelName = args[0];
        attrs = args[1];
