...
export default class HasMany extends Association {
  ...
  get identifier() {
    return `${camelize(this._container.inflector.singularize(this.key))}Ids`;
  }
   ...
   */
  getForeignKey() {
    return `${this._container.inflector.singularize(camelize(this.key))}Ids`;
  }
  ...
  */
  disassociateAllDependentsFromTarget(model) {
    ...
    let dependents = this.schema[this.schema.toCollectionName(owner)]
