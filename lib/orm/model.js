  ...
  _disassociateFromHasManyInverses(association) {
    ...
    if (tempAssociation && associateIds) {
      ...
      if (association.isPolymorphic) {
        ...
        models = this._schema[this._schema.toCollectionName(association.modelName)]
          .find(associateIds || [])
          .models;
      }
  ...
  */
  _disassociateFromBelongsToInverse(association) {
    ...
    if ((tempAssociation !== undefined) && associateId) {
      let associate;
      if (association.isPolymorphic) {
        associate = this._schema[this._schema.toCollectionName(associateId.type)]
          .find(associateId.id);
      } else {
        associate = this._schema[this._schema.toCollectionName(association.modelName)]
          .find(associateId);
      }
  ...
  _associateModelWithInverse(model, association) {
    if (model.hasInverseFor(association)) {
      ...
      if (inverse instanceof BelongsTo) {
        ...
        this._schema.db[this._schema.toInternalCollectionName(model.modelName)].update(model.id, { [inverseFk]: newId });
      } else {
