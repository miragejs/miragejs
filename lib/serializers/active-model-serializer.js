...
export default Serializer.extend({
  ...
  keyForRelationship(type) {
    return this._server.inflector.pluralize(underscore(type));
  },
  ...
  keyForRelationshipIds(type) {
    return `${underscore(this._server.inflector.singularize(type))}_ids`;
  },
  ...
  normalize(payload) {
    ...
    let jsonApiPayload = {
      data: {
        type: this._server.inflector.pluralize(type),
        attributes: {}
      }
    };
    if (attrs.id) {
