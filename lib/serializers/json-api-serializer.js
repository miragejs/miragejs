...
class JSONAPISerializer extends Serializer {
  ...
  */
  _relationshipIsIncludedForModel(relationshipKey, model) {
    if (this.hasQueryParamIncludes()) {
      ...
      // Check includes
      } else if (graph.included[this._server.inflector.pluralize(model.modelName)]) {
        graphResource = graph.included[this._server.inflector.pluralize(model.modelName)][graphKey];
      }
  ...
  */
  ...
  _addResourceToRequestedIncludesGraph(graph, resource, relationshipNames) {
    ...
    models.forEach(model => {
      let collectionName = this._server.inflector.pluralize(model.modelName);
  ...
  */
  typeKeyForModel(model) {
    return dasherize(this._server.inflector.pluralize(model.modelName));
  }
