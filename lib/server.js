...
/**
 * Default inflector with the base methods that throw an error
 */
const defaultInflector = {
  singularize() {
    throw '[singularize] No inflector was passed to Server - @miragejs/server';
  },
  pluralize() {
    throw '[pluralize] No inflector was passed to Server - @miragejs/server';
  }
};
...
export default class Server {
  ...
  config(config = {}) {
    ...
    */
    ...
    if (this.db) {
      ...
      this.db = new Db(undefined, config.identityManagers, this);
    }
    ...
    if (this.schema) {
      ...
      this.serializerOrRegistry = new SerializerRegistry(this.schema, config.serializers, this);
    }
  ...
  create(type, ...options) {
    ...
    if (this.schema && this.schema[toCollectionName(type)]) {
      ...
      if (collectionFromCreateList) {
        ...
        collectionName = this.schema ? toInternalCollectionName(type) : `_${this.inflector.pluralize(type)}`;
  ...
  */
  createList(type, amount, ...traitsAndOverrides) {
    ...
    let list = [];
    let collectionName = this.schema ? toInternalCollectionName(type) : `_${this.inflector.pluralize(type)}`;
