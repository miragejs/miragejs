...
/**
  ...
  Note that `this` within your `baseConfig` function refers to the server instance, which is the same instance that `server` refers to in your tests.
...
export default class Server {
  ...
  config(config = {}) {
    ...
    */
    ...
    /**
      Use `testConfig` to define routes that only apply in your test environment:
  ...
  */
  ...
  /**
    ...
    You can pass a function to `passthrough` to do a runtime check on wether or not the request should be handled by Mirage. If the function returns `true` Mirage will not handle the request and let it pass through.
