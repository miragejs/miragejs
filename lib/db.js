...
class Db {
  ...
  constructor(initialData, identityManagers, server) {
    this._collections = [];
  ...
  createCollection(name, initialData) {
    if (!this[name]) {
      ...
      let newCollection = new DbCollection(name, initialData, IdentityManager, this._server);
  ...
  identityManagerFor(name) {
    return this._identityManagers[this._server.inflector.singularize(name)] || this._identityManagers.application || IdentityManager;
  }
