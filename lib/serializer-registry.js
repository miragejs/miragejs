...
export default class SerializerRegistry {
  ...
  serialize(response, request) {
    ...
    if (this._isModelOrCollection(response)) {
      ...
      return response.reduce((json, collection) => {
        ...
        if (serializer.embed) {
          json[this._server.inflector.pluralize(collection.modelName)] = serializer.serialize(collection, request);
        } else {
  ...
  serializerFor(type, { explicit = false } = {}) {
    ...
    return new SerializerForResponse(this, type, this.request, this._server);
  }
